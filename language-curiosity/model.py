...
class TextCNN(nn.Module):
    def __init__(self, embedding_dim, n_filters, filter_sizes, output_dim,
        ...
        self.fig_feature = nn.Sequential(
            nn.Conv2d(
                in_channels=3,
                out_channels=32,
                kernel_size=8,
        ...
        self.classfier = nn.Linear(256, 2)
    ...
    def forward(self, x, state):
        ...
        feat = self.fig_feature(state)
        s7 = time.time()

        cat2 = torch.cat((cat, feat), dim=1)
        s8 = time.time()
        print("s1 s2 s3 s4 s5 s6 s7 s8 time is ", s1-s, s2-s1, s3-s2, s4-s3, s5-s4,s6-s5,s7-s6,s8-s7)

        return self.classfier(cat2)
